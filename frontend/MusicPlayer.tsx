...
const CDPlayer: React.FC = () => {
  ...
  const displayTrack = tracks.length
    ? `[${String(currentTrack + 1).padStart(2, "0")}] ${formatTime(time)}`
    : "[00] 00:00";
  const titleLabel = tracks.length
    ? tracks[currentTrack].name
    : "Please insert an audio compact disc.";
  const trackLabel = tracks.length ? String(currentTrack + 1) : "";
  ...
  const handleMenuClick = (e: ReactMouseEvent, menuKey: string) => {
    e.stopPropagation(); // Prevent closing immediately
    setOpenMenu(openMenu === menuKey ? null : menuKey);
  };

  // Generic menu item click handler
  const handleMenuItemClick = (e: ReactMouseEvent, action: () => void) => {
    e.stopPropagation();
    action();
      ...
      >
        {topMenus.map((menu) => (
          <div key={menu} className="menubar-menu">
            <span onClick={(e) => handleMenuClick(e, menu)}>{menu}</span>
            {openMenu === menu && (
